...
const write = (file, content) => {
  fs.writeFileSync(path.resolve('C:\\dev\\my-portfolio', file), content, 'utf8');
...
// Replace project card specifically
let pCardStart = '<GlassCard className="h-full flex flex-col">';
if (projects.includes(pCardStart)) {
  let parts = projects.split(pCardStart);
  let second = parts[1];
  let closeIdx = second.indexOf('</GlassCard>');
  second = second.slice(0, closeIdx) + '</Tilt3DCard>' + second.slice(closeIdx + 12);
  projects = parts[0] + '<Tilt3DCard className="h-full flex flex-col p-6 rounded-xl bg-[#12121A]/60 backdrop-blur-sm border border-[#1F1F29]">' + second;
}
...
let cCardStart = '<GlassCard className="h-full flex flex-col justify-between overflow-hidden border border-[#1F1F29] hover:border-[#4CC9F0]/50 transition-all group">';
if (certs.includes(cCardStart)) {
  let parts = certs.split(cCardStart);
  let second = parts[1];
  let closeIdx = second.indexOf('</GlassCard>');
  second = second.slice(0, closeIdx) + '</Tilt3DCard>' + second.slice(closeIdx + 12);
  certs = parts[0] + '<Tilt3DCard className="h-full flex flex-col justify-between overflow-hidden border border-[#1F1F29] hover:border-[#4CC9F0]/50 transition-all group bg-[#12121A]/60 rounded-xl">' + second;
}
...
// Skill cards: use Tilt3DCard 
let skillStart = '<GlassCard key={key} className="p-5">';
if (resume.includes(skillStart)) {
  let parts = resume.split(skillStart);
  let newRes = parts[0];
  for (let i = 1; i < parts.length; i++) {
    let second = parts[i];
    let closeIdx = second.indexOf('</GlassCard>');
    second = second.slice(0, closeIdx) + '</Tilt3DCard>' + second.slice(closeIdx + 12);
    newRes += '<Tilt3DCard key={key} className="p-5 bg-[#12121A]/60 rounded-xl border border-[#1F1F29]">' + second;
  }
  resume = newRes;
}
...
let hCardStart = '<GlassCard className="p-6 cursor-pointer hover:bg-[#121218]/80 transition-colors">';
if (hackathons.includes(hCardStart)) {
  let parts = hackathons.split(hCardStart);
  let newHack = parts[0];
  for (let i = 1; i < parts.length; i++) {
    let second = parts[i];
    let closeIdx = second.indexOf('</GlassCard>');
    second = second.slice(0, closeIdx) + '</Tilt3DCard>' + second.slice(closeIdx + 12);
    newHack += '<Tilt3DCard className="p-6 cursor-pointer hover:bg-[#121218]/80 transition-colors bg-[#12121A]/60 rounded-xl border border-[#1F1F29]">' + second;
  }
  hackathons = newHack;
}
...
let carCardStart = '<GlassCard className="p-6 hover:bg-[#121218]/60 transition-all duration-300">';
if (career.includes(carCardStart)) {
  let parts = career.split(carCardStart);
  let newCar = parts[0];
  for (let i = 1; i < parts.length; i++) {
    let second = parts[i];
    let closeIdx = second.indexOf('</GlassCard>');
    second = second.slice(0, closeIdx) + '</Tilt3DCard>' + second.slice(closeIdx + 12);
    newCar += '<Tilt3DCard className="p-6 hover:bg-[#121218]/60 transition-all duration-300 bg-[#12121A]/60 rounded-xl border border-[#1F1F29]">' + second;
  }
  career = newCar;
}
